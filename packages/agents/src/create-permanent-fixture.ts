 ...
 * Run with: bun --env-file=../../.env src/create-permanent-fixture.ts
 */
...
async function createPermanentFixture() {
  ...
  console.log(`   Downloaded GLB URL: ${assetResult.url}\n`);
  ...
  const webPublicDir = path.join(rootDir, 'apps/web/public/uploads/assets');
  const agentPublicDir = path.join(rootDir, 'packages/agents/public/uploads/assets');
  ...
  // Find candidate source GLB file on disk
  const filenameOnly = path.basename(assetResult.url);
  const candidates = [
    path.join(rootDir, 'apps/web/public/uploads/assets', filenameOnly),
    path.join(rootDir, 'packages/agents/public/uploads/assets', filenameOnly),
    path.join(rootDir, 'packages/agents', assetResult.url),
    path.join(rootDir, 'apps/web', assetResult.url),
  ];

  const sourcePath = candidates.find(c => fs.existsSync(c));
  if (!sourcePath) {
    throw new Error(`Downloaded asset file not found in any of candidate locations: ${candidates.join(', ')}`);
  }

  console.log(`   Found source file on disk at: ${sourcePath}`);
